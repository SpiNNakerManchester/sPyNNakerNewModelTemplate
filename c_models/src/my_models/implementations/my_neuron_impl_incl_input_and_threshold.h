...
static bool neuron_impl_initialise(uint32_t n_neurons)
{
    ...
    log_debug(
        ...
        "input type size = %u", n_neurons,
        sizeof(neuron_t), sizeof(input_type_t));
...
static bool neuron_impl_do_timestep_update(timer_t time, index_t neuron_index)
{
    ...
    for (int i=0; i<NUM_EXCITATORY_RECEPTORS; i++) {
    	exc_input[i] = exc_value[i] * neuron->my_multiplicator;
    ...
    state_t result = neuron_model_state_update(
			...
			NUM_INHIBITORY_RECEPTORS, inh_input, external_bias, neuron);
